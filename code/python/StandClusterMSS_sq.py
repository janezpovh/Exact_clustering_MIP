            ...
            if (i < size):
                for j in range(i+1,size):
                    if (Assignment[i] == Assignment[j]):
                        Avg_Dist_WC[Cluster_index[Assignment[i]]] = Avg_Dist_WC[Cluster_index[Assignment[i]]] + Dist_EU[i][j]*Dist_EU[i][j]
...
#    print(Avg_Dist_FC)
        ...
        for cl_i in range(cluster_i):
            DistSmall = np.zeros((Cluster_sizes[cl_i],Cluster_sizes[cl_i]))
...
#                print(cl_i," ",i," ",j," ",Cluster_sizes[cl_i]," ",Kmed_small_ind[i]," ",Kmed_small_ind[j])
                    DistSmall[i][j] = Dist_EU[Kmed_small_ind[i]][Kmed_small_ind[j]]*Dist_EU[Kmed_small_ind[i]][Kmed_small_ind[j]]
        
#        print(small_kmed(DistSmall))
            cent_act = Kmed_small_ind[small_kmed(DistSmall)]
...
#    print(Centers)
        ...
        Avg_Dist_FC = 0
        for i in range(size):
            Avg_Dist_FC = Avg_Dist_FC + Dist_EU[i][Centers[i]]*Dist_EU[i][Centers[i]]

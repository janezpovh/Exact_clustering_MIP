            ...
            if (i < size):
                for j in range(i+1,size):
                    if (Assignment[i] == Assignment[j]):
                        Avg_Dist_WC[Cluster_index[Assignment[i]]] = Avg_Dist_WC[Cluster_index[Assignment[i]]] + GeodDist[i][j]*GeodDist[i][j]
...
#                print(cl_i," ",i," ",j," ",Cluster_sizes[cl_i]," ",Kmed_small_ind[i]," ",Kmed_small_ind[j])
                    GeodDistSmall[i][j] = GeodDist[Kmed_small_ind[i]][Kmed_small_ind[j]]*GeodDist[Kmed_small_ind[i]][Kmed_small_ind[j]]
...
#    print(Centers)
        ...
        Avg_Dist_FC = 0
        for i in range(size):
            Avg_Dist_FC = Avg_Dist_FC + GeodDist[i][Centers[i]]*GeodDist[i][Centers[i]]

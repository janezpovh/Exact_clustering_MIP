...
#    prob.solve()
    ...
    for i in range(size):
        Avg_Dist_FC = Avg_Dist_FC + GeodDist[i,Assign[i][cluster_i-Cl_min]-1]*GeodDist[i,Assign[i][cluster_i-Cl_min]-1]
        if (i < size):
            for j in range(i+1,size):
                if (Assign[i][cluster_i-Cl_min] == Assign[j][cluster_i-Cl_min]):
                    Avg_Dist_WC[Cluster_index[Assign[i][cluster_i-Cl_min]-1]] = Avg_Dist_WC[Cluster_index[Assign[i][cluster_i-Cl_min]-1]] + GeodDist[i][j]*GeodDist[i][j]
